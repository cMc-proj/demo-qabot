...
function appendMessage(sender, text) {
  ...
  if (sender === "You") {
    div.classList.add("cmc-user");
  } else {
    div.classList.add("cmc-bot");
  }
...
async function sendMessage() {
  ...
  try {
    ...
    if (data.error) {
      ...
      appendMessage(INDUSTRIES[activeIndustry], data.reply);
    } else if (data.answer) {
      appendMessage(INDUSTRIES[activeIndustry], data.answer);
    } else {
      appendMessage(INDUSTRIES[activeIndustry], "⚠️ Unexpected response format.");
    }
  } catch (err) {
    messagesContainer.removeChild(loading);
